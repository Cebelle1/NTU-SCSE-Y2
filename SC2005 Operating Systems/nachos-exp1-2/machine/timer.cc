...
Timer::TimerExpired() 
{
    ...
    (*handler)(arg);	//calls the function to handle when timer expired, hence we do not need this when we want to just update timer
}
...
Timer::ScheduleTimerInterrupt(){
	...
	interrupt->Schedule(TimerHandler, (_int) this, TimeOfNextInterrupt(), 
		TimerInt); //updates the timer only
}
